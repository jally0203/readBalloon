...
def find_entities_in_layout(layout, doc, target_color, transform=Matrix44(), parent_color=None, balloons=None, dim_texts=None):
    ...
    for entity in layout:
        ...
        # ==========================================
            ...
            # 若含有匿名圖塊，遞迴導出內部圖元
            if entity.dxf.hasattr('geometry'):
                block_name = entity.dxf.geometry
                if block_name in doc.blocks:
                    dim_block = doc.blocks[block_name]
                    try:
                        dim_matrix = entity.matrix44()
                        combined_transform = dim_matrix * transform
                    except Exception:
                        combined_transform = transform

                    find_entities_in_layout(
                        layout=dim_block, doc=doc, target_color=target_color,
                        transform=combined_transform, parent_color=eff_color,
            ...
            # 1. 讀取測量值，強制固定為小數點第 2 位 (例如: 414.23 或 10.00)
            ...
            # 2. 讀取公差 (先看物件本身，若無則向 doc.dimstyles 繼承)
            ...
            # 組合公差字串，強制格式化為小數點第 2 位 (例如 ±0.15 或 (+0.15/-0.05))
            tol_str = ""
...
def is_valid_range_geometry(b1, b2, cfg):
    ...
    if abs(b1['rot'] - 0.0) < 5.0 or abs(b1['rot'] - 180.0) < 5.0:
        if dy > cfg['ORTHO_TOL']:
            return False
    elif abs(b1['rot'] - 90.0) < 5.0 or abs(b1['rot'] - 270.0) < 5.0:
        if dx > cfg['ORTHO_TOL']:
            return False
